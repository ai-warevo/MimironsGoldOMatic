...
| **A3** | `appsettings` + docs | Document **`Twitch:HelixApiBaseUrl`** in [`appsettings.Development.json`](../src/MimironsGoldOMatic.Backend/appsettings.Development.json); align with [`docs/E2E_AUTOMATION_PLAN.md`](E2E_AUTOMATION_PLAN.md). | Backend Dev | 0.25 d | A2 | **Done** |
...
| **C4** | Verification hooks | `GET /last-run` after `POST /run-sequence`; scripts in [`scripts/tier_b_verification/`](../scripts/tier_b_verification/). | Backend Dev | 0.5 d | C2 | Done (readiness) |
