...
1. Update **`docs/overview/SPEC.md`** as single source of truth: remove JSON file-bridge; define **`[MGM_WHO]`** + JSON on one log line; unified **`WoWChatLog.txt`** tail; addon-only winner whisper; Desktop-only **`confirm-acceptance`**; real Twitch JWT for Dev Rig + deploy; 30s grace for **`verify-candidate`**; Extension backoff/Retry; **`active_payout_exists`** without auto-expire; pause N/A MVP; **`spinPhase`** transitions Backend-defined.
2. Align **`README.md`**, **`CONTEXT.md`**, **`AGENTS.md`**, **`docs/overview/ROADMAP.md`**, **`docs/overview/INTERACTION_SCENARIOS.md`**, **`docs/reference/IMPLEMENTATION_READINESS.md`**, component **`ReadME.md`** files under **`docs/MimironsGoldOMatic.*/`**.
