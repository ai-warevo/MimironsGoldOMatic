...
- **HTTP:** Typed `EbsDesktopClient` wrapping `HttpClient` + Polly retry on transient/`429`/`503` (aligned with `docs/MimironsGoldOMatic.Desktop/ReadME.md`).
- **Settings:** JSON file + **DPAPI**-protected API key file under `%LocalAppData%\MimironsGoldOMatic\`.
- **Win32:** `IWoWInputStrategy` — `PostMessageWoWInputStrategy` (WM_CHAR to focused WoW main HWND), `SendInputWoWInputStrategy` (Unicode `SendInput`). Foreground `WoW.exe` via `GetForegroundWindow` + process name check (`docs/SPEC.md` §8).
- **Log tail:** One `FileStream` tail of `WoWChatLog.txt` (default: `{WoWInstall}\Logs\WoWChatLog.txt`, optional override path from settings). Parse tags per `docs/SPEC.md` §10; idempotent handling with in-memory keys.
...
- `docs/IMPLEMENTATION_READINESS.md`, `docs/ROADMAP.md` (MVP-4 status), `docs/MimironsGoldOMatic.Desktop/ReadME.md`
