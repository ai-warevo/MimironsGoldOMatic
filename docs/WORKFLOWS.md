...
Operational sequence at product level. **Normative detail:** [`docs/SPEC.md`](SPEC.md), [`docs/INTERACTION_SCENARIOS.md`](INTERACTION_SCENARIOS.md).

## 1. Enrollment

A **subscriber** types **`!twgold <CharacterName>`** in **broadcast** Twitch chat (**prefix case-insensitive**). The **EBS** ingests via **EventSub** and adds or updates that viewer in the **participant pool** if **`CharacterName`** is valid and **unique** among other viewers. Channel Points are **not** used. Optional Extension **`POST /api/payouts/claim`** mirrors pool rules for Dev Rig — see [`docs/SPEC.md`](SPEC.md) §5.
...
When the EBS creates a **`Pending`** payout, the **Extension** can show **“You won”**. Desktop injects **`/run NotifyWinnerWhisper("<payoutId>","<CharacterName>")`**; the **addon** sends the §9 **winner notification whisper** (Russian body per [`docs/SPEC.md`](SPEC.md) §9).
...
Contributors and AI agents follow [`AGENTS.md`](../AGENTS.md): history under **`docs/prompts/history/`**, **plan** / **checks** / **report** per task. **MVP** steps and prompts: [`docs/ROADMAP.md`](ROADMAP.md).
