...
├── docs/
│   ├── overview/           (SPEC, ROADMAP, ARCHITECTURE, INTERACTION_SCENARIOS, MVP_PRODUCT_SUMMARY)
│   ├── e2e/                (E2E automation plan/tasks, Tier B checklists)
│   ├── components/         (backend, desktop, twitch-extension, wow-addon, shared — ReadME + UI_SPEC)
│   ├── setup/              (SETUP.md, SETUP-for-developer.md, SETUP-for-streamer.md)
│   ├── reference/          (UI_SPEC hub, PROJECT_STRUCTURE, WORKFLOWS, GLOSSARY, IMPLEMENTATION_READINESS)
│   ├── ReadME.md           (engineering index + navigation)
