...
namespace MimironsGoldOMatic.Backend.Controllers;
...
public sealed class RouletteController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Claim([FromBody] MimironsGoldOMatic.Shared.CreatePayoutRequest body, CancellationToken ct)
    {
        var uid = TwitchUserId();
        if (string.IsNullOrEmpty(uid))
            return Unauthorized();
