...
namespace MimironsGoldOMatic.Backend.Application;
    ...
    : IRequestHandler<PatchGiftRequestCommand, HandlerResult<GiftRequestDto>>
{
    public async Task<HandlerResult<GiftRequestDto>> Handle(PatchGiftRequestCommand request, CancellationToken ct)
    {
        ...
        row.FailureReason = request.State == GiftRequestState.Failed ? request.Reason ?? "failed" : null;
        if (request.State == GiftRequestState.SelectingItem)
            row.TimeoutAt = DateTime.UtcNow.Add(GiftQueueService.SelectingTimeout);
        else if (request.State == GiftRequestState.WaitingConfirmation)
            row.TimeoutAt = DateTime.UtcNow.Add(GiftQueueService.WaitingConfirmationTimeout);
        else if (request.State == GiftRequestState.Completed || request.State == GiftRequestState.Failed)
            row.TimeoutAt = null;
        session.Store(row);
